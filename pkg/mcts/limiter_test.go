...
func TestLimiterSingleLimits(t *testing.T) {
	...
	if !limiter.Ok(1000000, 1, 1) || !limiter.Expand() {
	...
	if ok := limiter.Ok(10, 1, 1); ok {
		t.Errorf("<Size=%d: ok=%v, want=%v", 10, ok, !ok)
	}

	if ok := limiter.Ok(9, 1, 1); !ok {
...
func TestLimiterCombos(t *testing.T) {
	...
	limiter.SetLimits(DefaultLimits().SetByteSize(32 * 10))
	limiter.Reset()

	// Nodes + memory limit, if memory exhausted, wait for 'nodes' and disable expanding
	// if !(limiter.Ok(10, 1, 1) && !limiter.Expand()) {
	// 	t.Error(">Nodes+Memory failed: ok=", limiter.Ok(10, 1, 1), "expand=", limiter.Expand())
	// }
	// if !(!limiter.Ok(10, 1, 1) && !limiter.Expand()) {
	// 	t.Error("<Nodes+Memory failed: ok=", limiter.Ok(10, 1, 1), "expand=", limiter.Expand())
	// }
	...
	if !(limiter.Ok(100, 1, 1) && !limiter.Expand()) {
		t.Error(">Time+Memory failed: ok=", limiter.Ok(100, 1, 1), "expand=", limiter.Expand())
	}
